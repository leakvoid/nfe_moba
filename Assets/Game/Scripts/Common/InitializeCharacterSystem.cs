...
public partial struct InitializeCharacterSystem : ISystem
{
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (var (physicsMass, mobaTeam, newCharacterEntity)
            in SystemAPI.Query<RefRW<PhysicsMass>, MobaTeam>().WithAny<NewChampTag, NewMinionTag>().WithEntityAccess())
        {
            ...
            {
                Common.TeamType.Blue => new float4(0, 0, 0, 1),
                Common.TeamType.Red => new float4(1, 0, 0, 1),
                _ => new float4(1),
            };
